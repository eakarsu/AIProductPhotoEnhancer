...
// Upload raw product photo; AI auto-enhances (color correction, exposure, sharpness); removes/replaces background; generates multiple lifestyle variants; outputs all formats
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductPhotoEnhancer / cf-ai-photo-enhancement-pipeline',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_ai_photo_enhancement_pipeline`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'AI photo enhancement pipeline'.
Context: Upload raw product photo; AI auto-enhances (color correction, exposure, sharpness); removes/replaces background; generates multiple lifestyle variants; outputs all formats
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-ai-photo-enhancement-pipeline/run error:', err);
