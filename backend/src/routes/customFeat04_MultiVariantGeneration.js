...
// Single product photo; AI generates lifestyle variants (on model, in room, close-up); generates marketing copy per image
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductPhotoEnhancer / cf-multi-variant-generation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_multi_variant_generation`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Multi-variant generation'.
Context: Single product photo; AI generates lifestyle variants (on model, in room, close-up); generates marketing copy per image
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-multi-variant-generation/run error:', err);
