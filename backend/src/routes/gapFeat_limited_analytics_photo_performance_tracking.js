...
// Gap Feature: Limited analytics (photo performance tracking)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductPhotoEnhancer / gap-limited-analytics-photo-performance-tracking',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited analytics (photo performance tracking)'.\nProject: AIProductPhotoEnhancer.\nContext: Limited analytics (photo performance tracking)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-analytics-photo-performance-tracking', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-analytics-photo-performance-tracking', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
