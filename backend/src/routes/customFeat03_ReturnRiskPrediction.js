...
// Product photo quality scoring; AI identifies poorly-lit, unclear, or suspicious photos that increase return likelihood; recommends re-shoot
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductPhotoEnhancer / cf-return-risk-prediction',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_return_risk_prediction`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Return risk prediction'.
Context: Product photo quality scoring; AI identifies poorly-lit, unclear, or suspicious photos that increase return likelihood; recommends re-shoot
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-return-risk-prediction/run error:', err);
