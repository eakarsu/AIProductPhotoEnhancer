...
// Product photo + reference object (coin, hand); AI estimates dimensions; provides size chart recommendations based on category (clothing, furniture, etc.)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductPhotoEnhancer / cf-computer-vision-product-sizing',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_computer_vision_product_sizing`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Computer vision product sizing'.
Context: Product photo + reference object (coin, hand); AI estimates dimensions; provides size chart recommendations based on category (clothing, furniture, etc.)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-computer-vision-product-sizing/run error:', err);
