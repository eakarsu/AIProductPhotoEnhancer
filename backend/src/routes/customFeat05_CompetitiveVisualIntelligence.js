...
// User uploads competitor product photos; AI analyzes presentation style (background, lighting, angles); recommends matching or differentiation strategies
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductPhotoEnhancer / cf-competitive-visual-intelligence',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_competitive_visual_intelligence`;
try {
  await pool.query(`
...
router.post('/run', authMiddleware, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Competitive visual intelligence'.
Context: User uploads competitor product photos; AI analyzes presentation style (background, lighting, angles); recommends matching or differentiation strategies
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-competitive-visual-intelligence/run error:', err);
