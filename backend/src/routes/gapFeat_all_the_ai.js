...
// Gap Feature: All the AI
// All the AI-themed route files exist as stubs — none expose actual AI endpoints in TSV
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIProductPhotoEnhancer / gap-all-the-ai',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'All the AI'.\nProject: AIProductPhotoEnhancer.\nContext: All the AI-themed route files exist as stubs — none expose actual AI endpoints in TSV\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['all-the-ai', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'all-the-ai', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
