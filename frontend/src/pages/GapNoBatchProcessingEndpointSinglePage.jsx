...
// Gap (Non-AI): No batch processing endpoint (single
// No batch processing endpoint (single-image only)
import React, { useState } from 'react';

export default function GapNoBatchProcessingEndpointSinglePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-batch-processing-endpoint-single/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No batch processing endpoint (single</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No batch processing endpoint (single-image only)</p>
