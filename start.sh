...
(cd "$PROJECT_DIR/backend"&&BACKEND_PORT="$BACKEND_PORT" node src/index.js)&CHILD_PIDS+=("$!")
(cd "$PROJECT_DIR/frontend"&&npm run dev -- --port "$FRONTEND_PORT" --host 127.0.0.1)&CHILD_PIDS+=("$!")
echo "Product photo services started without installing, seeding, migrating, or reclaiming ports."
wait "${CHILD_PIDS[@]}"
